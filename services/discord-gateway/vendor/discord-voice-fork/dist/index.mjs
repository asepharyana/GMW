    ...
    if (this.state.code === 0 /* OpeningWs */) {
      this.state.ws.sendPacket({
        ...
        d: {
          ...
          // [GMW FORK] Declare video capability so Discord sends RTP video
          // (camera + screen-share) for all members. Derived from
          // Discord-RE/Discord-video-stream which uses a video-capable
          // Identify. stream = [] because this client is a pure receiver
          // (does not send its own video). Gated by DISCORD_VOICE_ENABLE_VIDEO.
          video: true,
          streams: []
        }
      });
...
var SSRCMap = class extends EventEmitter6 {
  ...
  get(target) {
    if (typeof target === "number") {
      // [GMW FORK] Also match a video SSRC (camera/screen-share) when the
      // audio SSRC key does not match. Video RTP packets carry a different
      // SSRC than the audio stream, so look it up across stored values.
      const direct = this.map.get(target);
      if (direct) return direct;
      for (const data of this.map.values()) {
        if (data.videoSSRC === target) {
          return data;
        }
      }
      return void 0;
...
var VoiceReceiver = class {
  ...
  onWsPacket(packet) {
    if (packet.op === VoiceOpcodes3.ClientDisconnect) {
      ...
      // [GMW FORK] Also register the user's video SSRC when present so the
      // UDP receive path can decrypt/demux their H264 (camera/screen-share).
      // op 12 (Speaking) carries `streams: [{ type, rid, quality, active,
      // ssrc, rtx_ssrc, video_ssrc }]` when the member enables camera/
      // screen-share. We map the video stream SSRC -> userId alongside the
      // audio SSRC.
      let videoSSRC;
      const streams = packet.d && Array.isArray(packet.d.streams) ? packet.d.streams : [];
      for (const st of streams) {
        const s = st && (st.ssrc ?? st.video_ssrc);
        if (typeof s === "number" && s !== 0) {
          videoSSRC = s;
          break;
        }
      }
      const update = { userId: packet.d.user_id, audioSSRC: packet.d.ssrc };
      if (typeof videoSSRC === "number") update.videoSSRC = videoSSRC;
      this.ssrcMap.update(update);
    }
  }
